...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<Map<String, String>> _viralRecipes = [
    {'title': 'Pizza Keju Lumer', 'chef': 'Chef Renata', 'rating': '4.9', 'time': '25m', 'img': 'https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?w=600&auto=format&fit=crop&q=60'},
    {'title': 'Salmon Salad Premium', 'chef': 'Dr. Reisa', 'rating': '4.8', 'time': '15m', 'img': 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=600&auto=format&fit=crop&q=60'},
    {'title': 'Sate Ayam Madura Asli', 'chef': 'Cak Umar', 'rating': '4.9', 'time': '30m', 'img': 'https://images.unsplash.com/photo-1529042410759-befb1204b468?w=600&auto=format&fit=crop&q=60'},
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // DRAWER TETAP ADA DAN MAKIN RAPI
      drawer: Drawer(
        child: ListView(
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    const Text('Rekomendasi Hari Ini', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black87)),
                    Text('Lihat Semua', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.redAccent.shade400)),
                  ],
                ),
              ),
              const SizedBox(height: 15),
              ...
              Padding(
                ...
                child: GestureDetector(
                  onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const RecipeDetailScreen(title: 'Ramen Kuah Pedas Buldak', imgUrl: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=800&auto=format&fit=crop&q=80'))),
                  child: Hero(
                    tag: 'hero_image_Ramen Kuah Pedas Buldak', // Efek transisi ke layar detail
                    child: Container(
                      height: 200, width: double.infinity,
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(24),
                        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.15), blurRadius: 15, offset: const Offset(0, 8))],
                        image: const DecorationImage(image: NetworkImage('https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=800&auto=format&fit=crop&q=80'), fit: BoxFit.cover),
                      ),
                      child: Container(
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(24),
                          gradient: LinearGradient(colors: [Colors.black.withOpacity(0.8), Colors.transparent], begin: Alignment.bottomCenter, end: Alignment.topCenter, stops: const [0.0, 0.7]),
                        ),
                        ...
                        child: Column(
                          ...
                          children: [
                            Container(
                              ...
                              child: const Text('🔥 SPESIAL', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1)),
                            ),
                            const SizedBox(height: 8),
                            const Text('Ramen Kuah Pedas Buldak', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold, decoration: TextDecoration.none)),
                            const SizedBox(height: 4),
                            Row(
                              children: const [
                                Icon(Icons.schedule_rounded, color: Colors.white70, size: 16), SizedBox(width: 4),
                                Text('45 Menit', style: TextStyle(color: Colors.white70, fontSize: 13, decoration: TextDecoration.none)), SizedBox(width: 15),
                                Icon(Icons.local_fire_department_rounded, color: Colors.white70, size: 16), SizedBox(width: 4),
                                Text('Tingkat Sulit', style: TextStyle(color: Colors.white70, fontSize: 13, decoration: TextDecoration.none)),
                              ],
                            ),
                          ],
                        ),
              ...
              // SEDANG VIRAL SECTION
              const Padding(
                padding: EdgeInsets.symmetric(horizontal: 20),
                child: Text('Sedang Viral 🔥', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black87)),
              ),
              const SizedBox(height: 15),
              ...
              SizedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return GestureDetector(
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => RecipeDetailScreen(title: item['title']!, imgUrl: item['img']!))),
                      child: Container(
                        width: 180,
                        margin: const EdgeInsets.only(right: 16, bottom: 20), // Bottom margin for shadow
                        decoration: BoxDecoration(
                          color: Colors.white,
                          borderRadius: BorderRadius.circular(20),
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(item['title']!, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.black87), maxLines: 1, overflow: TextOverflow.ellipsis),
                                  const SizedBox(height: 4),
                                  Text('Oleh ${item['chef']}', style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                                  const SizedBox(height: 12),
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Row(children: [const Icon(Icons.star_rounded, color: Colors.amber, size: 16), Text(' ${item['rating']}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold))]),
                                      Row(children: [const Icon(Icons.schedule_rounded, color: Colors.grey, size: 14), Text(' ${item['time']}', style: const TextStyle(fontSize: 11, color: Colors.grey))]),
                                    ],
                                  )
                                ],
                              ),
              ...
              const SizedBox(height: 80), // Padding extra di bawah agar tidak tertutup BottomNavBar
            ],
          ),
        ),
      ),
